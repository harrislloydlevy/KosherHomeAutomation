...
/**
 ...
 18 Tishrey 5763 Hol hamoed Sukot" ) formated date.

 @param h The hdate_struct of the date to print.
 @param buf The buffer to write the string into. Returned value may be pointer to this buffer
 @param len The length of the buffer provided
...
char * hdate_get_format_date (hdate_struct const *h, char *buf, size_t len, int const diaspora, int const short_format)
{
	...
	size_t hebrew_buffer_len; // Used to track the current length or the string to avoid going over
	...
	************************************************************/
	if (short_format)
	{
		hebrew_buffer_len = snprintf(buf, len, "%s %s %s\n",
				hday_int_str,
	...
	************************************************************/
	...
	{
		hebrew_buffer_len = snprintf (buf, len, "%s %s%s %s",
				hday_int_str,
				bet_h,
				hdate_string( HDATE_STRING_HMONTH , h->hd_mon, HDATE_STRING_LONG, hebrew_format),
				hyear_int_str);
		...
		if (hebrew_buffer_len != -1) omer_day = hdate_get_omer_day(h);
		if (omer_day != 0)
		{
			size_t added_length;
			...
			added_length =
				snprintf
				(
					buf+hebrew_buffer_len-1, // The -1 here is so we we write over the null terminator.
					len - hebrew_buffer_len,
					", %s of the Omer",
					omer_str);
			// Snprintf returns the number of charachters that *would* have been written if there was
			// enough space. If we run out of space however we just kill the whole procedure.
			if (added_length < (len - hebrew_buffer_len)) {
				hebrew_buffer_len = -1;				
			} else {
				hebrew_buffer_len += added_length;
			}
		}
		
		/* if holiday print it */
		if (hebrew_buffer_len == -1) holiday = hdate_get_holyday (h, diaspora);
		if (holiday != 0)
		{
			size_t added_length;
			char *holiday_str = hdate_string( HDATE_STRING_HOLIDAY, holiday, HDATE_STRING_LONG, hebrew_format);
	...
	if (hebrew_buffer_len != -1) return buf;
	return NULL;
}
...
char* hdate_string( int const type_of_string, int const index, int const input_short_form, int const input_hebrew_form)
{
	...
	case HDATE_STRING_OMER:
				if (index > 0 && index < 50)
				{
					...
					omer_str_len = strlen(h_int_string) + strlen(" in the Omer");
					return_string = malloc(omer_str_len);
					
					return_string_len = snprintf(
							return_string, omer_str_len, "%s%s", h_int_string, _(" in the Omer"));
	...
	case HDATE_STRING_INT:
				if ((index > 0) && (index < 11000))
				{
					...
					if (!hebrew_form)
					{
						return_string_len = snprintf (return_string, HEBREW_NUMBER_BUFFER_SIZE, "%d", index);
